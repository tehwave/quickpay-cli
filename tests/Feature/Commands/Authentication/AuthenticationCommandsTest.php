...
it('can fail an unauthenticated readiness check without contacting quickpay', function () {
    Http::fake();

    $this->artisan('auth', ['--check' => true])
        ->expectsOutputToContain('Scope: not authenticated')
        ->assertExitCode(1);
...
it('writes unauthenticated status as json for automation', function () {
    Http::fake();
    $command = new AuthCommand;
    $command->setLaravel(app());
    $tester = new CommandTester($command);

    $status = $tester->execute(['--json' => true], ['capture_stderr_separately' => true]);

    expect($status)->toBe(0)
        ->and(json_decode($tester->getDisplay(), true, flags: JSON_THROW_ON_ERROR))->toBe([
            'authenticated' => false,
            'credential_source' => null,
            'api_base_url' => 'https://api.quickpay.net',
            'api_version' => 'v10',
            'scope' => null,
        ])
...
it('writes authenticated status as json and passes a readiness check', function () {
    file_put_contents($this->credentialPath, json_encode(['api_key' => 'stored-secret'], JSON_THROW_ON_ERROR));
    Http::fake(['https://api.quickpay.net/ping' => Http::response(['scope' => 'merchant', 'version' => 'v10'])]);
    $command = new AuthCommand;
    $command->setLaravel(app());
    ...
    expect($status)->toBe(0)
        ->and(json_decode($tester->getDisplay(), true, flags: JSON_THROW_ON_ERROR))->toBe([
            'authenticated' => true,
            'credential_source' => 'config file',
            'api_base_url' => 'https://api.quickpay.net',
            'api_version' => 'v10',
            'scope' => 'merchant',
        ])
        ->and($tester->getErrorOutput())->toBe('')
        ->and($tester->getDisplay())->not->toContain('stored-secret');
});
...
it('routes command failures to stderr', function () {
    ...
    expect($status)->toBe(1)
        ->and($tester->getErrorOutput())->toContain('Authentication check failed: HTTP 401:')
