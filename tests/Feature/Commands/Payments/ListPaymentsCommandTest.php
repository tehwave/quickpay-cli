...
use App\Commands\Payments\ListPaymentsCommand;
...
it('renders a useful human payment table', function () {
    ...
    expect($tester->execute([]))->toBe(0)
        ...
        ->toContain('Order ID')
        ->toContain('Balance / amount (minor units)')
        ->toContain('order-12')
        ->toContain('processed')
        ->toContain('2500');
});

it('tells human users when more payment pages are available', function () {
    Http::fake(['https://api.quickpay.net/payments*' => Http::response(
        [['id' => 12]],
        200,
    ...
    $this->artisan('payments:list')
        ->expectsOutputToContain('More payments are available. Re-run with --all')
        ->assertExitCode(0);

    Http::assertSentCount(1);
});

it('keeps a next-page notice on stderr in json mode', function () {
    $raw = '[{"id":12}]';
    Http::fake(['https://api.quickpay.net/payments*' => Http::response(
        $raw,
        200,
    ...
    expect($status)->toBe(0)
        ...
        ->and($tester->getErrorOutput())->toContain('More payments are available')
        ->toContain('--all');
});
...
it('sanitizes credentials and terminal controls in list json and human output', function () {
    ...
    $jsonCommand = new ListPaymentsCommand;
    ...
    $humanCommand = new ListPaymentsCommand;
...
it('fails safely with a login hint when credentials are missing', function () {
    putenv('QUICKPAY_API_KEY');
    $file = new CredentialFile(sys_get_temp_dir().'/missing-quickpay-'.bin2hex(random_bytes(6)).'.json');
