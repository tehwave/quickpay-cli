...
it('allows only validated quickpay pagination urls', function () {
    Http::fake(['https://api.quickpay.net/payments?page=2' => Http::response([])]);

    $client = new QuickpayClient(app(Factory::class), 'secret');

    expect($client->getPagination('https://api.quickpay.net/payments?page=2')->successful())->toBeTrue();
    expect(fn () => $client->getPagination('https://example.com/payments?page=2'))
        ->toThrow(InvalidArgumentException::class, 'Quickpay');
    expect(fn () => $client->getPagination('https://attacker@api.quickpay.net/payments?page=2'))
        ->toThrow(InvalidArgumentException::class, 'Quickpay');
});
