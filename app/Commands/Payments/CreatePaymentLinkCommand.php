...
class CreatePaymentLinkCommand extends AuthenticatedCommand
{
    ...
    protected $signature = 'payments:link
        {id : Quickpay payment ID}
        {amount : Link amount as a positive integer in minor units}
        {--continue-url= : Browser destination after successful payment}
        {--cancel-url= : Browser destination after cancelled payment}
        {--callback-url= : Callback URL for Quickpay servers (not localhost)}
        {--language= : Payment-window language}
        {--payment-methods= : Allowed payment methods}
        {--auto-capture : Request automatic capture}
        {--field=* : Additional request field as key=value; repeatable}
