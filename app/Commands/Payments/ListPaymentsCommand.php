...
class ListPaymentsCommand extends AuthenticatedCommand
{
    protected $signature = 'payments:list
        {--accepted : Include only accepted payments}
        {--state= : Include only payments in this state}
        {--order-id= : Include only payments with this order ID}
        {--created-after= : Minimum created_at timestamp}
        {--created-before= : Maximum created_at timestamp}
        {--page-size=20 : Payments per page, from 1 through 100}
        {--all : Fetch every available page}
        {--max-pages=100 : Maximum pages fetched with --all}
        ...
        return $this->withQuickpay($quickpay, function (AuthenticatedQuickpay $authenticated) use ($presenter): int {
            ...
            if (! $this->option('all')) {
                ...
                if ($this->option('json')) {
                    ...
                    $this->writeMorePaymentsNotice($page->response);
    ...
    private function writePayments(array $payments, string $apiKey, PaymentPresenter $presenter): void
    {
        ...
        $this->table(
            ['ID', 'Order ID', 'Accepted', 'State', 'Currency', 'Balance / amount (minor units)', 'Created'],
            $presenter->payments($payments, $apiKey),
        );
    }

    private function writeMorePaymentsNotice(QuickpayResponse $response): void
    {
        try {
            $hasNextPage = LinkHeaderParser::next($response->header('Link')) !== null;
        ...
        $output->writeln(
            'More payments are available. Re-run with --all to fetch every page.',
